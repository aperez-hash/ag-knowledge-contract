...
const objectPath = path.join(rootDir, "examples", "irnr-establecimientos-permanentes.json");
const STORY_GENERATED_AT = "2026-07-19T23:59:00Z";

export const DERIVED_DIR = path.join(rootDir, "examples", "derived");
export const PLANNER_VIEW_PATH = path.join(DERIVED_DIR, "irnr-establecimientos-permanentes-planner-view.json");
export const WEB_VIEW_PATH = path.join(DERIVED_DIR, "irnr-establecimientos-permanentes-web-view.json");
export const AI_CONTEXT_PATH = path.join(DERIVED_DIR, "irnr-establecimientos-permanentes-ai-context.json");
export const CHECKLIST_VIEW_PATH = path.join(DERIVED_DIR, "irnr-establecimientos-permanentes-checklist.json");
export const FAQ_VIEW_PATH = path.join(DERIVED_DIR, "irnr-establecimientos-permanentes-faq.json");
export const CLIENT_RESPONSE_PATH = path.join(DERIVED_DIR, "irnr-establecimientos-permanentes-client-response.json");
export const VALIDATION_REPORT_PATH = path.join(
  DERIVED_DIR,
  "irnr-establecimientos-permanentes-validation-report.json",
);
export const INCIDENT_LOG_PATH = path.join(DERIVED_DIR, "irnr-establecimientos-permanentes-incident-log.json");
...
export function validateStorySemanticRules(knowledgeObject) {
  ...
  const requiredModelCodes = ["200", "202", "206"];
  ...
  const requiredRelationTargets = [
    ...
    ["ganancias_patrimoniales", "ganancias"],
  ];
  ...
  if (
    ...
    errors.push("Debe existir al menos una relacion marcada como 'Consumes Doctrine From:'.");
  }
  ...
  if (
    ...
    errors.push("Debe existir al menos una relacion marcada como 'Provides Doctrine To:'.");
  }

  const technicalBlock = getBlock(knowledgeObject, "technical_development");
  if (technicalBlock && !/establecimiento permanente|\bEP\b/iu.test(technicalBlock.content.development)) {
    errors.push("El desarrollo tecnico debe tratar expresamente el establecimiento permanente.");
  }

  if (technicalBlock && !/lugar fijo|agente/iu.test(technicalBlock.content.development)) {
    errors.push("El desarrollo tecnico debe tratar expresamente lugar fijo de negocios o agentes.");
  }

  if (technicalBlock && !/preparatoria o auxiliar|preparatorio o auxiliar/iu.test(technicalBlock.content.development)) {
    errors.push("El desarrollo tecnico debe tratar expresamente la frontera de actividad preparatoria o auxiliar.");
  }

  if (technicalBlock && !/CDI|convenio/iu.test(technicalBlock.content.development)) {
    errors.push("El desarrollo tecnico debe tratar expresamente el convenio aplicable.");
  }

  if (technicalBlock && !/Modelo 200|modelo 200/u.test(technicalBlock.content.development)) {
    errors.push("El desarrollo tecnico debe citar expresamente el Modelo 200 como capa procedimental derivada.");
  }

  if (technicalBlock && !/Modelo 202|modelo 202/u.test(technicalBlock.content.development)) {
    errors.push("El desarrollo tecnico debe citar expresamente el Modelo 202 como capa de pagos fraccionados.");
  }

  if (technicalBlock && !/Modelo 206|modelo 206/u.test(technicalBlock.content.development)) {
    errors.push("El desarrollo tecnico debe citar expresamente el Modelo 206 como documento de ingreso o devolucion.");
  }

  if (technicalBlock && !/Modelo OCDE|Comentarios/iu.test(technicalBlock.content.development)) {
    errors.push("El desarrollo tecnico debe tratar expresamente la funcion interpretativa del Modelo OCDE y sus Comentarios.");
  }

  if (technicalBlock && !/atribucion detallada de beneficios|Precios de Transferencia|Impuesto sobre Sociedades/iu.test(technicalBlock.content.development)) {
    errors.push("El desarrollo tecnico debe acotar expresamente lo que escala a IS, atribucion de beneficios o Precios de Transferencia.");
  }
  ...
  if (
    requiredDocumentationBlock &&
    !requiredDocumentationBlock.content.items.some((item) => /convenio|CDI|residencia/iu.test(item.label + item.notes))
  ) {
    errors.push("La matriz de evidencias debe incluir soporte de residencia o de convenio.");
  }
  ...
  if (
    ...
    errors.push("La matriz de evidencias debe incluir certificado de residencia fiscal.");
  }

  if (
    requiredDocumentationBlock &&
    !requiredDocumentationBlock.content.items.some((item) => /poderes|agente|contratar/iu.test(item.label + item.notes))
  ) {
    errors.push("La matriz de evidencias debe incluir soporte sobre agentes, poderes o capacidad para contratar.");
  }
...
export async function buildStoryArtifacts() {
  ...
  return {
    ...
    validationReport: {
      ...
      generatedAt: STORY_GENERATED_AT,
