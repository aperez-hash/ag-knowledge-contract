...
const objectPath = path.join(rootDir, "examples", "modelo-210-imputacion-rentas.json");

export const DERIVED_DIR = path.join(rootDir, "examples", "derived");
export const PLANNER_VIEW_PATH = path.join(DERIVED_DIR, "modelo-210-planner-view.json");
export const WEB_VIEW_PATH = path.join(DERIVED_DIR, "modelo-210-web-view.json");
export const AI_CONTEXT_PATH = path.join(DERIVED_DIR, "modelo-210-ai-context.json");
export const VALIDATION_REPORT_PATH = path.join(
  DERIVED_DIR,
  "modelo-210-validation-report.json",
);
export const INCIDENT_LOG_PATH = path.join(DERIVED_DIR, "modelo-210-incident-log.json");
...
export function validateStorySemanticRules(knowledgeObject) {
  ...
  if (!knowledgeObject.affectedTaxModels.some((model) => model.modelCode === "210")) {
    errors.push("affectedTaxModels debe incluir el Modelo 210 como modelo primario.");
  }

  if (!knowledgeObject.affectedForms.some((form) => form.formCode === "210")) {
    errors.push("affectedForms debe incluir el formulario oficial 210.");
  }

  const faqBlock = getBlock(knowledgeObject, "faq");
  if (faqBlock && faqBlock.content.items.length < 4) {
    errors.push("El bloque FAQ debe incluir al menos 4 preguntas frecuentes reales.");
  }

  const caseBlock = getBlock(knowledgeObject, "case_study");
  if (caseBlock && caseBlock.content.items.length < 3) {
    errors.push("El bloque de casos practicos debe incluir al menos 3 casos.");
  }
