...
const objectPath = path.join(rootDir, "examples", "residencia-fiscal-espana.json");

export const DERIVED_DIR = path.join(rootDir, "examples", "derived");
export const PLANNER_VIEW_PATH = path.join(DERIVED_DIR, "residencia-fiscal-planner-view.json");
export const WEB_VIEW_PATH = path.join(DERIVED_DIR, "residencia-fiscal-web-view.json");
export const AI_CONTEXT_PATH = path.join(DERIVED_DIR, "residencia-fiscal-ai-context.json");
export const CHECKLIST_VIEW_PATH = path.join(DERIVED_DIR, "residencia-fiscal-checklist.json");
export const FAQ_VIEW_PATH = path.join(DERIVED_DIR, "residencia-fiscal-faq.json");
export const CLIENT_RESPONSE_PATH = path.join(DERIVED_DIR, "residencia-fiscal-client-response.json");
export const VALIDATION_REPORT_PATH = path.join(
  DERIVED_DIR,
  "residencia-fiscal-validation-report.json",
);
export const INCIDENT_LOG_PATH = path.join(DERIVED_DIR, "residencia-fiscal-incident-log.json");
...
export function validateStorySemanticRules(knowledgeObject) {
  ...
  if (
    !knowledgeObject.relations.every((relation) =>
      relation.rationale.startsWith("Provides Residency Decision To:"),
    )
  ) {
    errors.push("Todas las relaciones deben explicitar la categoria funcional 'Provides Residency Decision To:'.");
  }

  const technicalBlock = getBlock(knowledgeObject, "technical_development");
  if (
    technicalBlock &&
    !/criterio principal/u.test(technicalBlock.content.development)
  ) {
    errors.push("El desarrollo tecnico debe identificar expresamente el criterio principal.");
  }

  if (
    technicalBlock &&
    !/criterio alternativo/u.test(technicalBlock.content.development)
  ) {
    errors.push("El desarrollo tecnico debe identificar expresamente el criterio alternativo.");
  }

  if (
    technicalBlock &&
    !/presuncion familiar/u.test(technicalBlock.content.development)
  ) {
    errors.push("El desarrollo tecnico debe tratar expresamente la presuncion familiar.");
  }
  ...
  if (technicalBlock && !/183 dias/u.test(technicalBlock.content.development)) {
    errors.push("El desarrollo tecnico debe recoger expresamente el umbral de 183 dias.");
  }

  if (technicalBlock && !/centro de intereses economicos/u.test(technicalBlock.content.development)) {
    errors.push("El desarrollo tecnico debe tratar expresamente el centro de intereses economicos.");
  }
  ...
  if (
    requiredDocumentationBlock &&
    !requiredDocumentationBlock.content.items.some((item) => /registro de entradas y salidas/iu.test(item.label))
  ) {
    errors.push("La matriz de evidencias debe incluir registros de entradas y salidas.");
  }

  if (
    requiredDocumentationBlock &&
    !requiredDocumentationBlock.content.items.some((item) => /consumos|escolarizacion/u.test(item.label + item.notes))
  ) {
    errors.push("La matriz de evidencias debe incluir consumos, escolarizacion u otras pruebas de vida efectiva.");
  }
