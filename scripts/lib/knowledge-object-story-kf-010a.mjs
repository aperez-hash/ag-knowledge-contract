...
export const WEB_VIEW_PATH = path.join(DERIVED_DIR, "irnr-rendimientos-capital-inmobiliario-web-view.json");
export const AI_CONTEXT_PATH = path.join(DERIVED_DIR, "irnr-rendimientos-capital-inmobiliario-ai-context.json");
export const CHECKLIST_VIEW_PATH = path.join(DERIVED_DIR, "irnr-rendimientos-capital-inmobiliario-checklist.json");
export const FAQ_VIEW_PATH = path.join(DERIVED_DIR, "irnr-rendimientos-capital-inmobiliario-faq.json");
export const CLIENT_RESPONSE_PATH = path.join(DERIVED_DIR, "irnr-rendimientos-capital-inmobiliario-client-response.json");
export const VALIDATION_REPORT_PATH = path.join(
  DERIVED_DIR,
  "irnr-rendimientos-capital-inmobiliario-validation-report.json",
);
export const INCIDENT_LOG_PATH = path.join(DERIVED_DIR, "irnr-rendimientos-capital-inmobiliario-incident-log.json");
...
export function validateStorySemanticRules(knowledgeObject) {
  ...
  if (
    technicalBlock &&
    !/inmueble situado en Espana/u.test(technicalBlock.content.development)
  ) {
    errors.push("El desarrollo tecnico debe tratar expresamente el inmueble situado en Espana.");
  }

  if (
    technicalBlock &&
    !/capital inmobiliario/u.test(technicalBlock.content.development)
  ) {
    errors.push("El desarrollo tecnico debe tratar expresamente el capital inmobiliario.");
  }

  if (
    technicalBlock &&
    !/actividad economica/u.test(technicalBlock.content.development)
  ) {
    errors.push("El desarrollo tecnico debe tratar expresamente la frontera con actividad economica.");
  }

  if (
    technicalBlock &&
    !/UE\/EEE|UE o en un Estado del EEE|UE o en un Estado del Espacio Economico Europeo/u.test(technicalBlock.content.development)
  ) {
    errors.push("El desarrollo tecnico debe tratar expresamente el tratamiento UE/EEE.");
  }

  if (technicalBlock && !/terceros Estados|tercer Estado/u.test(technicalBlock.content.development)) {
    errors.push("El desarrollo tecnico debe recoger expresamente el contraste con terceros Estados.");
  }
  ...
  if (technicalBlock && !/gastos/u.test(technicalBlock.content.development)) {
    errors.push("El desarrollo tecnico debe tratar expresamente la deducibilidad de gastos.");
  }

  if (technicalBlock && !/copropiedad/u.test(technicalBlock.content.development)) {
    errors.push("El desarrollo tecnico debe tratar expresamente la copropiedad.");
  }
  ...
  if (
    requiredDocumentationBlock &&
    !requiredDocumentationBlock.content.items.some((item) => /titularidad|participacion|copropiedad/iu.test(item.label + item.notes))
  ) {
    errors.push("La matriz de evidencias debe incluir soporte de titularidad, participacion o copropiedad.");
  }
  ...
  if (
    requiredDocumentationBlock &&
    !requiredDocumentationBlock.content.items.some((item) => /gasto|deduc/iu.test(item.label + item.notes))
  ) {
    errors.push("La matriz de evidencias debe incluir soporte de gastos deducibles o de su analisis.");
  }
