...
const objectPath = path.join(rootDir, "examples", "modelo-714-impuesto-patrimonio.json");

export const DERIVED_DIR = path.join(rootDir, "examples", "derived");
export const PLANNER_VIEW_PATH = path.join(DERIVED_DIR, "modelo-714-planner-view.json");
export const WEB_VIEW_PATH = path.join(DERIVED_DIR, "modelo-714-web-view.json");
export const AI_CONTEXT_PATH = path.join(DERIVED_DIR, "modelo-714-ai-context.json");
export const CHECKLIST_VIEW_PATH = path.join(DERIVED_DIR, "modelo-714-checklist.json");
export const FAQ_VIEW_PATH = path.join(DERIVED_DIR, "modelo-714-faq.json");
export const CLIENT_RESPONSE_PATH = path.join(DERIVED_DIR, "modelo-714-client-response.json");
export const VALIDATION_REPORT_PATH = path.join(
  DERIVED_DIR,
  "modelo-714-validation-report.json",
);
export const INCIDENT_LOG_PATH = path.join(DERIVED_DIR, "modelo-714-incident-log.json");
...
export function validateStorySemanticRules(knowledgeObject) {
  ...
  if (!knowledgeObject.affectedTaxModels.some((model) => model.modelCode === "714")) {
    errors.push("affectedTaxModels debe incluir el Modelo 714 como modelo primario.");
  }

  if (!knowledgeObject.affectedForms.some((form) => form.formCode === "714")) {
    errors.push("affectedForms debe incluir el formulario oficial 714.");
  }
  ...
  if (caseBlock && caseBlock.content.items.length < 4) {
    errors.push("El bloque de casos practicos debe incluir al menos 4 casos.");
  }
  ...
  if (!knowledgeObject.relations.some((relation) => relation.targetKnowledgeObjectId.includes("721"))) {
    errors.push("Debe existir relacion expresa con el objeto previsto del Modelo 721 para delimitar alcance.");
  }

  if (!knowledgeObject.relations.some((relation) => relation.targetKnowledgeObjectId.includes("itsgf"))) {
    errors.push("Debe existir relacion expresa con el objeto previsto de ITSGF para reflejar la conexion operativa.");
  }

  const technicalBlock = getBlock(knowledgeObject, "technical_development");
  if (
    technicalBlock &&
    !/obligacion personal/u.test(technicalBlock.content.development)
  ) {
    errors.push("El desarrollo tecnico debe tratar expresamente la obligacion personal.");
  }

  if (
    technicalBlock &&
    !/obligacion real/u.test(technicalBlock.content.development)
  ) {
    errors.push("El desarrollo tecnico debe tratar expresamente la obligacion real.");
  }

  if (
    technicalBlock &&
    !/2\.000\.000/u.test(technicalBlock.content.development)
  ) {
    errors.push("El desarrollo tecnico debe recoger el umbral de 2.000.000 euros para obligacion de declarar.");
  }
