...
const objectPath = path.join(rootDir, "examples", "modelo-720-bienes-extranjero.json");

export const DERIVED_DIR = path.join(rootDir, "examples", "derived");
export const PLANNER_VIEW_PATH = path.join(DERIVED_DIR, "modelo-720-planner-view.json");
export const WEB_VIEW_PATH = path.join(DERIVED_DIR, "modelo-720-web-view.json");
export const AI_CONTEXT_PATH = path.join(DERIVED_DIR, "modelo-720-ai-context.json");
export const CHECKLIST_VIEW_PATH = path.join(DERIVED_DIR, "modelo-720-checklist.json");
export const FAQ_VIEW_PATH = path.join(DERIVED_DIR, "modelo-720-faq.json");
export const CLIENT_RESPONSE_PATH = path.join(DERIVED_DIR, "modelo-720-client-response.json");
export const VALIDATION_REPORT_PATH = path.join(
  DERIVED_DIR,
  "modelo-720-validation-report.json",
);
export const INCIDENT_LOG_PATH = path.join(DERIVED_DIR, "modelo-720-incident-log.json");
...
export function validateStorySemanticRules(knowledgeObject) {
  ...
  if (!knowledgeObject.affectedTaxModels.some((model) => model.modelCode === "720")) {
    errors.push("affectedTaxModels debe incluir el Modelo 720 como modelo primario.");
  }

  if (!knowledgeObject.affectedForms.some((form) => form.formCode === "720")) {
    errors.push("affectedForms debe incluir el formulario oficial 720.");
  }
