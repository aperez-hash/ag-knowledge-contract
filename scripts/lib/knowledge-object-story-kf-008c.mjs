...
const objectPath = path.join(
  rootDir,
  "examples",
  "modelo-151-regimen-trabajadores-desplazados.json",
);

export const DERIVED_DIR = path.join(rootDir, "examples", "derived");
export const PLANNER_VIEW_PATH = path.join(DERIVED_DIR, "modelo-151-planner-view.json");
export const WEB_VIEW_PATH = path.join(DERIVED_DIR, "modelo-151-web-view.json");
export const AI_CONTEXT_PATH = path.join(DERIVED_DIR, "modelo-151-ai-context.json");
export const CHECKLIST_VIEW_PATH = path.join(DERIVED_DIR, "modelo-151-checklist.json");
export const FAQ_VIEW_PATH = path.join(DERIVED_DIR, "modelo-151-faq.json");
export const CLIENT_RESPONSE_PATH = path.join(DERIVED_DIR, "modelo-151-client-response.json");
export const VALIDATION_REPORT_PATH = path.join(
  DERIVED_DIR,
  "modelo-151-validation-report.json",
);
export const INCIDENT_LOG_PATH = path.join(DERIVED_DIR, "modelo-151-incident-log.json");
...
export function validateStorySemanticRules(knowledgeObject) {
  ...
  const requiredModelCodes = ["149", "151", "210"];
  ...
  const requiredRelationTargets = ["residencia", "cdi", "210", "irpf", "irnr", "fiscalidad"];

  for (const targetFragment of requiredRelationTargets) {
    if (!knowledgeObject.relations.some((relation) => relation.targetKnowledgeObjectId.includes(targetFragment))) {
      errors.push(`Debe existir una relacion expresa que apunte a '${targetFragment}'.`);
    }
  }

  const consumesDoctrineRelations = knowledgeObject.relations.filter((relation) =>
    relation.rationale.startsWith("Consumes Doctrine From:"),
  );
  if (consumesDoctrineRelations.length < 2) {
    errors.push("El objeto debe declarar de forma expresa que consume doctrina de Residencia Fiscal y CDI.");
  }

  const technicalBlock = getBlock(knowledgeObject, "technical_development");
  if (technicalBlock && !/Decision Gateway/u.test(technicalBlock.content.development)) {
    errors.push("El desarrollo tecnico debe identificar expresamente el Decision Gateway.");
  }

  if (technicalBlock && !/acceso/u.test(technicalBlock.content.development)) {
    errors.push("El desarrollo tecnico debe tratar expresamente la fase de acceso al regimen.");
  }

  if (technicalBlock && !/permanencia/u.test(technicalBlock.content.development)) {
    errors.push("El desarrollo tecnico debe tratar expresamente la fase de permanencia en el regimen.");
  }

  if (technicalBlock && !/Cronologia operativa/u.test(technicalBlock.content.development)) {
    errors.push("El desarrollo tecnico debe incluir la cronologia operativa del regimen.");
  }

  if (technicalBlock && !/familiares|conyuge|hijos/u.test(technicalBlock.content.development)) {
    errors.push("El desarrollo tecnico debe tratar la capa de familiares como dominio nuclear.");
  }

  if (technicalBlock && !/Delegated Analysis/u.test(technicalBlock.content.development)) {
    errors.push("El desarrollo tecnico debe declarar expresamente el analisis delegado.");
  }

  if (technicalBlock && !/Modelo 149/u.test(technicalBlock.content.development)) {
    errors.push("El objeto debe tratar expresamente el Modelo 149 como puerta de entrada.");
  }

  if (technicalBlock && !/Modelo 151/u.test(technicalBlock.content.development)) {
    errors.push("El objeto debe tratar expresamente el Modelo 151 como pieza anual de gestion.");
  }
  ...
  if (
    ...
    errors.push("La documentacion necesaria debe incluir certificado de residencia fiscal anterior.");
  }

  if (
    requiredDocumentationBlock &&
    !requiredDocumentationBlock.content.items.some((item) => /familiares|parentesco|discapacidad/iu.test(item.label))
  ) {
    errors.push("La documentacion necesaria debe incluir soporte especifico de familiares.");
  }

  const procedureBlock = getBlock(knowledgeObject, "procedure");
  if (
    procedureBlock &&
    !procedureBlock.content.steps.some((step) => /Modelo 149/u.test(step.text))
  ) {
    errors.push("El procedimiento debe incluir la preparacion del Modelo 149.");
  }

  if (
    procedureBlock &&
    !procedureBlock.content.steps.some((step) => /Modelo 151/u.test(step.text))
  ) {
    errors.push("El procedimiento debe incluir la gestion anual del Modelo 151.");
  }

  if (
    procedureBlock &&
    !procedureBlock.content.steps.some((step) => /familiares/u.test(step.text))
  ) {
    errors.push("El procedimiento debe incluir el analisis operativo de familiares.");
  }
