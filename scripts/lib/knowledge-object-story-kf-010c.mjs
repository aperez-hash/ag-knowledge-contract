...
export const VALIDATION_REPORT_PATH = path.join(
  DERIVED_DIR,
  "irnr-ganancias-patrimoniales-validation-report.json",
);
...
export function validateStorySemanticRules(knowledgeObject) {
  ...
  const requiredModelCodes = ["210", "211"];
  ...
  if (technicalBlock && !/ganancia patrimonial/u.test(technicalBlock.content.development)) {
    errors.push("El desarrollo tecnico debe tratar expresamente la ganancia patrimonial.");
  }

  if (technicalBlock && !/acciones y participaciones|acciones o participaciones/u.test(technicalBlock.content.development)) {
    errors.push("El desarrollo tecnico debe tratar expresamente acciones y participaciones.");
  }

  if (technicalBlock && !/no sujecion/u.test(technicalBlock.content.development)) {
    errors.push("El desarrollo tecnico debe separar expresamente la no sujecion.");
  }

  if (technicalBlock && !/exencion/u.test(technicalBlock.content.development)) {
    errors.push("El desarrollo tecnico debe tratar expresamente las exenciones relevantes.");
  }
  ...
  if (technicalBlock && !/Modelo 211|modelo 211|3 por ciento/u.test(technicalBlock.content.development)) {
    errors.push("El desarrollo tecnico debe tratar expresamente la retencion del 3 por ciento y el Modelo 211.");
  }
  ...
  if (
    requiredDocumentationBlock &&
    !requiredDocumentationBlock.content.items.some((item) => /modelo 211|retencion del 3|retencion|exencion|cdi/iu.test(item.label + item.notes))
  ) {
    errors.push("La matriz de evidencias debe incluir soporte de retencion, CDI o exencion relevante.");
  }
