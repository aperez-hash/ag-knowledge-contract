...
const objectPath = path.join(rootDir, "examples", "modelo-721-monedas-virtuales.json");
...
export const VALIDATION_REPORT_PATH = path.join(
  DERIVED_DIR,
  "modelo-721-validation-report.json",
);
...
export function validateStorySemanticRules(knowledgeObject) {
  ...
  if (!knowledgeObject.affectedTaxModels.some((model) => model.modelCode === "721")) {
    errors.push("affectedTaxModels debe incluir el Modelo 721 como modelo primario.");
  }

  if (!knowledgeObject.affectedForms.some((form) => form.formCode === "721")) {
    errors.push("affectedForms debe incluir el formulario oficial 721.");
  }
  ...
  if (!knowledgeObject.relations.some((relation) => relation.targetKnowledgeObjectId.includes("modelo720"))) {
    errors.push("Debe existir relacion expresa con el Modelo 720 para delimitar correctamente el alcance.");
  }

  if (!knowledgeObject.relations.some((relation) => relation.targetKnowledgeObjectId.includes("cripto"))) {
    errors.push(
      "Debe existir relacion expresa con el futuro objeto de ganancias patrimoniales por criptoactivos para separar materias.",
    );
  }
  ...
  if (
    technicalBlock &&
    !/custodia/u.test(technicalBlock.content.development)
  ) {
    errors.push("El desarrollo tecnico debe tratar expresamente el concepto de custodia.");
  }

  if (
    technicalBlock &&
    !/autocustodia/u.test(technicalBlock.content.development)
  ) {
    errors.push("El desarrollo tecnico debe tratar expresamente la exclusion por autocustodia.");
  }

  if (
    technicalBlock &&
    !/50\.000/u.test(technicalBlock.content.development)
  ) {
    errors.push("El desarrollo tecnico debe recoger el umbral de 50.000 euros.");
  }

  if (
    technicalBlock &&
    !/20\.000/u.test(technicalBlock.content.development)
  ) {
    errors.push("El desarrollo tecnico debe recoger la regla del incremento superior a 20.000 euros.");
  }

  if (
    technicalBlock &&
    !/Modelo 720/u.test(technicalBlock.content.development)
  ) {
    errors.push("El desarrollo tecnico debe separar expresamente el Modelo 721 del Modelo 720.");
  }
