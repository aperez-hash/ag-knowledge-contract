...
const objectPath = path.join(rootDir, "examples", "irnr-dividendos-intereses-canones.json");
const STORY_GENERATED_AT = "2026-07-18T23:59:00Z";

export const DERIVED_DIR = path.join(rootDir, "examples", "derived");
export const PLANNER_VIEW_PATH = path.join(DERIVED_DIR, "irnr-dividendos-intereses-canones-planner-view.json");
export const WEB_VIEW_PATH = path.join(DERIVED_DIR, "irnr-dividendos-intereses-canones-web-view.json");
export const AI_CONTEXT_PATH = path.join(DERIVED_DIR, "irnr-dividendos-intereses-canones-ai-context.json");
export const CHECKLIST_VIEW_PATH = path.join(DERIVED_DIR, "irnr-dividendos-intereses-canones-checklist.json");
export const FAQ_VIEW_PATH = path.join(DERIVED_DIR, "irnr-dividendos-intereses-canones-faq.json");
export const CLIENT_RESPONSE_PATH = path.join(DERIVED_DIR, "irnr-dividendos-intereses-canones-client-response.json");
export const VALIDATION_REPORT_PATH = path.join(
  DERIVED_DIR,
  "irnr-dividendos-intereses-canones-validation-report.json",
);
export const INCIDENT_LOG_PATH = path.join(DERIVED_DIR, "irnr-dividendos-intereses-canones-incident-log.json");
...
export function validateStorySemanticRules(knowledgeObject) {
  ...
  const requiredModelCodes = ["210", "216"];
  ...
  if (technicalBlock && !/dividendo|interes|canon/iu.test(technicalBlock.content.development)) {
    errors.push("El desarrollo tecnico debe tratar expresamente dividendos, intereses o canones.");
  }

  if (technicalBlock && !/fuente espanola|norma interna/iu.test(technicalBlock.content.development)) {
    errors.push("El desarrollo tecnico debe tratar expresamente la fuente espanola conforme a norma interna.");
  }

  if (technicalBlock && !/exencion interna|exenciones internas/iu.test(technicalBlock.content.development)) {
    errors.push("El desarrollo tecnico debe tratar expresamente las exenciones internas.");
  }
  ...
  if (technicalBlock && !/retencion/iu.test(technicalBlock.content.development)) {
    errors.push("El desarrollo tecnico debe tratar expresamente la capa de retenciones.");
  }

  if (technicalBlock && !/Modelo 210|modelo 210/u.test(technicalBlock.content.development)) {
    errors.push("El desarrollo tecnico debe citar expresamente el Modelo 210 como capa procedimental derivada.");
  }

  if (technicalBlock && !/Modelo 216|modelo 216/u.test(technicalBlock.content.development)) {
    errors.push("El desarrollo tecnico debe citar expresamente el Modelo 216 como capa del retenedor.");
  }
  ...
  if (
    requiredDocumentationBlock &&
    !requiredDocumentationBlock.content.items.some((item) => /retencion|liquidacion|justificante de pago/iu.test(item.label + item.notes))
  ) {
    errors.push("La matriz de evidencias debe incluir soporte de retencion o del detalle del pago.");
  }
