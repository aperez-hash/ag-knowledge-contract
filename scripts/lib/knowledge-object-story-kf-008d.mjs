...
const objectPath = path.join(rootDir, "examples", "irnr-rendimientos-trabajo.json");

export const DERIVED_DIR = path.join(rootDir, "examples", "derived");
export const PLANNER_VIEW_PATH = path.join(DERIVED_DIR, "irnr-rendimientos-trabajo-planner-view.json");
export const WEB_VIEW_PATH = path.join(DERIVED_DIR, "irnr-rendimientos-trabajo-web-view.json");
export const AI_CONTEXT_PATH = path.join(DERIVED_DIR, "irnr-rendimientos-trabajo-ai-context.json");
export const CHECKLIST_VIEW_PATH = path.join(DERIVED_DIR, "irnr-rendimientos-trabajo-checklist.json");
export const FAQ_VIEW_PATH = path.join(DERIVED_DIR, "irnr-rendimientos-trabajo-faq.json");
export const CLIENT_RESPONSE_PATH = path.join(DERIVED_DIR, "irnr-rendimientos-trabajo-client-response.json");
export const VALIDATION_REPORT_PATH = path.join(
  DERIVED_DIR,
  "irnr-rendimientos-trabajo-validation-report.json",
);
export const INCIDENT_LOG_PATH = path.join(DERIVED_DIR, "irnr-rendimientos-trabajo-incident-log.json");
...
export function validateStorySemanticRules(knowledgeObject) {
  ...
  const requiredModelCodes = ["210", "216", "151"];
  ...
  if (
    technicalBlock &&
    !/teletrabajo internacional/u.test(technicalBlock.content.development)
  ) {
    errors.push("El desarrollo tecnico debe tratar expresamente el teletrabajo internacional.");
  }

  if (
    technicalBlock &&
    !/modelos hibridos/u.test(technicalBlock.content.development)
  ) {
    errors.push("El desarrollo tecnico debe tratar expresamente los modelos hibridos.");
  }

  if (
    technicalBlock &&
    !/retencion/u.test(technicalBlock.content.development)
  ) {
  ...
  if (
    technicalBlock &&
    !/empresa espanola/u.test(technicalBlock.content.development)
  ) {
    errors.push("El desarrollo tecnico debe tratar expresamente el caso de empresa espanola.");
  }

  if (technicalBlock && !/lugar de realizacion efectiva del trabajo|donde se presta el trabajo/u.test(technicalBlock.content.development)) {
    errors.push("El desarrollo tecnico debe recoger expresamente el lugar efectivo de prestacion del trabajo.");
  }
  ...
  if (
    requiredDocumentationBlock &&
    !requiredDocumentationBlock.content.items.some((item) => /registro de dias trabajados|calendario de presencia|trazabilidad de trabajo/iu.test(item.label))
  ) {
    errors.push("La matriz de evidencias debe incluir soporte de dias trabajados o trazabilidad del trabajo.");
  }
  ...
  if (
    requiredDocumentationBlock &&
    !requiredDocumentationBlock.content.items.some((item) => /retencion/iu.test(item.label + item.notes))
  ) {
    errors.push("La matriz de evidencias debe incluir soporte de retenciones o cierre declarativo.");
  }
