...
test("el Knowledge Object del Modelo 721 valida contra el schema publicado", async () => {
...
test("el objeto cumple las reglas semanticas de STORY-KF-005C", async () => {
  const knowledgeObject = await loadKnowledgeObject();
...
test("la vista de Planner mantiene el contenido operativo permitido", async () => {
  ...
  assert.equal(
    result.data.relevantRelations.some((relation) =>
      relation.targetKnowledgeObjectId.includes("modelo720"),
    ),
    true,
  );
});
...
test("el objeto mantiene un caso practico expreso de exchange espanol sin obligacion automatica", async () => {
  const knowledgeObject = await loadKnowledgeObject();
  const caseBlock = knowledgeObject.blocks.find((block) => block.type === "case_study");

  assert.equal(
    caseBlock.content.items.some((item) => /(exchange|proveedor) espanol/u.test(item.scenario)),
    true,
  );
});
...
test("el informe de validacion queda certificado completamente en PASS", async () => {
  ...
  assert.equal(artifacts.validationReport.story, "STORY-KF-005C");
...
test("los ficheros generados quedan sincronizados con la derivacion actual", async () => {
  ...
  assert.equal(validationReport.story, "STORY-KF-005C");
