...
} from "../scripts/lib/knowledge-object-story-kf-008a.mjs";
...
test("el Knowledge Object de Residencia Fiscal valida contra el schema publicado", async () => {
...
test("el objeto cumple las reglas semanticas de STORY-KF-008A", async () => {
  const knowledgeObject = await loadKnowledgeObject();
...
test("el objeto mantiene la jerarquia de criterios y la matriz de evidencias", async () => {
  const knowledgeObject = await loadKnowledgeObject();
  ...
  assert.equal(
    /criterio principal.*183 dias/u.test(technicalBlock.content.development),
    true,
  );
  assert.equal(
    /criterio alternativo.*centro de intereses economicos/u.test(technicalBlock.content.development),
    true,
  );
  assert.equal(/presuncion familiar/u.test(technicalBlock.content.development), true);
...
test("el informe de validacion queda certificado completamente en PASS", async () => {
  ...
  assert.equal(artifacts.validationReport.story, "STORY-KF-008A");
...
test("los ficheros generados quedan sincronizados con la derivacion actual", async () => {
  ...
  assert.equal(validationReport.story, "STORY-KF-008A");
