...
} from "../scripts/lib/knowledge-object-story-kf-008b.mjs";
...
test("el Knowledge Object de CDI valida contra el schema publicado", async () => {
...
test("el objeto cumple las reglas semanticas de STORY-KF-008B", async () => {
  const knowledgeObject = await loadKnowledgeObject();
...
test("la vista de Planner mantiene el contenido operativo permitido", async () => {
  ...
  assert.equal(result.data.relevantRelations.length, 8);
  assert.equal(
...
test("el contexto IA incluye criterio, procedimiento, riesgos y restricciones", async () => {
  ...
  assert.equal(result.data.procedure.length >= 9, true);
  assert.equal(result.data.risks.length, 5);
  assert.equal(result.data.clientResponseAuthorization.channelAccess, "derived_only");
  assert.equal(
    result.data.technicalDevelopment.some((entry) => /aplicabilidad/u.test(entry)),
    true,
  );
  assert.equal(
    result.data.technicalDevelopment.some((entry) => /interpretacion/u.test(entry)),
    true,
  );
});
...
test("el objeto mantiene el Decision Gateway y la matriz de evidencias", async () => {
  const knowledgeObject = await loadKnowledgeObject();
  const technicalBlock = knowledgeObject.blocks.find((block) => block.type === "technical_development");
  const procedureBlock = knowledgeObject.blocks.find((block) => block.type === "procedure");
  ...
  assert.equal(/interpretacion/u.test(technicalBlock.content.development), true);
  assert.equal(/tie-breaker/u.test(technicalBlock.content.development), true);
  assert.equal(/no crea por si sola una obligacion tributaria nueva/u.test(technicalBlock.content.development), true);
  assert.equal(
    procedureBlock.content.steps.some((step) => /CDI vigente/u.test(step.text)),
    true,
  );
  assert.equal(
...
test("el objeto declara dependencias doctrinales y mantiene fuera la granularidad material", async () => {
  const knowledgeObject = await loadKnowledgeObject();
  const technicalBlock = knowledgeObject.blocks.find((block) => block.type === "technical_development");
  ...
  assert.equal(
    knowledgeObject.relations.some((relation) => /Residencia Fiscal/u.test(relation.rationale)),
    true,
  );
  assert.equal(
    /dividendos|canones|rentas inmobiliarias|intereses financieros/u.test(
      technicalBlock.content.development,
    ),
    false,
  );
});
...
test("el informe de validacion queda certificado completamente en PASS", async () => {
  ...
  assert.equal(artifacts.validationReport.story, "STORY-KF-008B");
...
test("los ficheros generados quedan sincronizados con la derivacion actual", async () => {
  ...
  assert.equal(validationReport.story, "STORY-KF-008B");
