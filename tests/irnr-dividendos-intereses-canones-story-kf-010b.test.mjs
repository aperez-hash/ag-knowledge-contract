...
} from "../scripts/lib/knowledge-object-story-kf-010b.mjs";
...
test("el Knowledge Object de IRNR sobre dividendos, intereses y canones valida contra el schema publicado", async () => {
...
test("el objeto cumple las reglas semanticas de STORY-KF-010B", async () => {
  const knowledgeObject = await loadKnowledgeObject();
...
test("el objeto mantiene el foco en dividendos, intereses y canones, exenciones, convenio y matriz de evidencias", async () => {
  const knowledgeObject = await loadKnowledgeObject();
  ...
  assert.equal(/dividendo|interes|canon/u.test(technicalBlock.content.development), true);
  assert.equal(/exencion interna|exenciones internas/u.test(technicalBlock.content.development), true);
  ...
  assert.equal(/Modelo 216|modelo 216/u.test(technicalBlock.content.development), true);
  assert.equal(
  ...
  assert.equal(
    requiredDocumentationBlock.content.items.some((item) =>
      /retencion|justificantes de pago|liquidaciones/u.test(item.label + item.notes),
    ),
    true,
  );
});
...
test("el informe de validacion queda certificado completamente en PASS", async () => {
  ...
  assert.equal(artifacts.validationReport.story, "STORY-KF-010B");
...
test("los ficheros generados quedan sincronizados con la derivacion actual", async () => {
  ...
  assert.equal(validationReport.story, "STORY-KF-010B");
