...
} from "../scripts/lib/knowledge-object-story-kf-005b.mjs";
...
test("el Knowledge Object del Modelo 714 valida contra el schema publicado", async () => {
...
test("el objeto cumple las reglas semanticas de STORY-KF-005B", async () => {
  const knowledgeObject = await loadKnowledgeObject();
...
test("la vista de Planner mantiene el contenido operativo permitido", async () => {
  ...
  assert.equal(result.data.relevantRelations.length, 4);
  assert.equal(
  ...
  assert.equal(
    result.data.relevantRelations.some((relation) =>
      relation.targetKnowledgeObjectId.includes("itsgf"),
    ),
    true,
  );
});
...
test("el objeto mantiene un caso practico expreso de obligacion real", async () => {
  const knowledgeObject = await loadKnowledgeObject();
  const caseBlock = knowledgeObject.blocks.find((block) => block.type === "case_study");

  assert.equal(
    caseBlock.content.items.some((item) => /obligacion real/u.test(item.scenario)),
    true,
  );
});
...
test("el informe de validacion queda certificado completamente en PASS", async () => {
  ...
  assert.equal(artifacts.validationReport.story, "STORY-KF-005B");
...
test("los ficheros generados quedan sincronizados con la derivacion actual", async () => {
  ...
  assert.equal(validationReport.story, "STORY-KF-005B");
