...
} from "../scripts/lib/knowledge-object-story-kf-010c.mjs";
...
test("el Knowledge Object de IRNR sobre ganancias patrimoniales valida contra el schema publicado", async () => {
...
test("el objeto cumple las reglas semanticas de STORY-KF-010C", async () => {
  const knowledgeObject = await loadKnowledgeObject();
...
test("el objeto mantiene el foco en ganancias patrimoniales, no sujecion, exencion y retencion inmobiliaria", async () => {
  const knowledgeObject = await loadKnowledgeObject();
  ...
  assert.equal(/ganancia patrimonial/u.test(technicalBlock.content.development), true);
  assert.equal(/acciones y participaciones|acciones o participaciones/u.test(technicalBlock.content.development), true);
  assert.equal(/no sujecion/u.test(technicalBlock.content.development), true);
  assert.equal(/exencion/u.test(technicalBlock.content.development), true);
  assert.equal(/Modelo 210|modelo 210/u.test(technicalBlock.content.development), true);
  assert.equal(/Modelo 211|modelo 211|3 por ciento/u.test(technicalBlock.content.development), true);
  assert.equal(/copropiedad/u.test(technicalBlock.content.development), true);
  assert.equal(
  ...
  assert.equal(
    requiredDocumentationBlock.content.items.some((item) =>
      /modelo 211|retencion del 3|retencion|cdi|exencion/u.test(item.label + item.notes),
    ),
    true,
  );
});
...
test("el informe de validacion queda certificado completamente en PASS", async () => {
  ...
  assert.equal(artifacts.validationReport.story, "STORY-KF-010C");
...
test("los ficheros generados quedan sincronizados con la derivacion actual", async () => {
  ...
  assert.equal(validationReport.story, "STORY-KF-010C");
