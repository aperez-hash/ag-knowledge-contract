...
} from "../scripts/lib/knowledge-object-story-kf-005a.mjs";
...
test("el objeto cumple las reglas semanticas de STORY-KF-005A", async () => {
  const knowledgeObject = await loadKnowledgeObject();
...
test("la vista de Planner mantiene el contenido operativo permitido", async () => {
  ...
  assert.equal(result.data.relevantRelations.length, 3);
  assert.equal(
...
test("el informe de validacion queda certificado completamente en PASS", async () => {
  ...
  assert.equal(artifacts.validationReport.story, "STORY-KF-005A");
...
test("los ficheros generados quedan sincronizados con la derivacion actual", async () => {
  ...
  assert.equal(validationReport.story, "STORY-KF-005A");
