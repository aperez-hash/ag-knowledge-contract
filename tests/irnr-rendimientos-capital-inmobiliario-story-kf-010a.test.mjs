...
} from "../scripts/lib/knowledge-object-story-kf-010a.mjs";
...
test("el Knowledge Object de IRNR sobre rendimientos del capital inmobiliario valida contra el schema publicado", async () => {
...
test("el objeto cumple las reglas semanticas de STORY-KF-010A", async () => {
  const knowledgeObject = await loadKnowledgeObject();
...
test("el objeto mantiene el foco en capital inmobiliario, deducibilidad y matriz de evidencias", async () => {
  const knowledgeObject = await loadKnowledgeObject();
  ...
  assert.equal(/capital inmobiliario/u.test(technicalBlock.content.development), true);
  assert.equal(/actividad economica/u.test(technicalBlock.content.development), true);
  assert.equal(/UE\/EEE|Espacio Economico Europeo/u.test(technicalBlock.content.development), true);
  assert.equal(/terceros Estados|tercer Estado/u.test(technicalBlock.content.development), true);
...
test("el informe de validacion queda certificado completamente en PASS", async () => {
  ...
  assert.equal(artifacts.validationReport.story, "STORY-KF-010A");
...
test("los ficheros generados quedan sincronizados con la derivacion actual", async () => {
  ...
  assert.equal(validationReport.story, "STORY-KF-010A");
