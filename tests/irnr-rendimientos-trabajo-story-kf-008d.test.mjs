...
} from "../scripts/lib/knowledge-object-story-kf-008d.mjs";
...
test("el Knowledge Object de IRNR sobre rendimientos del trabajo valida contra el schema publicado", async () => {
...
test("el objeto cumple las reglas semanticas de STORY-KF-008D", async () => {
  const knowledgeObject = await loadKnowledgeObject();
...
test("la vista de Planner mantiene el contenido operativo permitido", async () => {
  ...
  assert.equal(result.data.relevantRelations.length, 7);
  assert.equal(
...
test("el objeto mantiene el foco en teletrabajo, modelos hibridos y matriz de evidencias", async () => {
  const knowledgeObject = await loadKnowledgeObject();
  ...
  assert.equal(/teletrabajo internacional/u.test(technicalBlock.content.development), true);
  assert.equal(/modelos hibridos/u.test(technicalBlock.content.development), true);
  assert.equal(/retencion/u.test(technicalBlock.content.development), true);
...
test("el informe de validacion queda certificado completamente en PASS", async () => {
  ...
  assert.equal(artifacts.validationReport.story, "STORY-KF-008D");
...
test("los ficheros generados quedan sincronizados con la derivacion actual", async () => {
  ...
  assert.equal(validationReport.story, "STORY-KF-008D");
