...
} from "../scripts/lib/knowledge-object-story-kf-008c.mjs";

async function readJson(filePath) {
  return JSON.parse(await readFile(filePath, "utf8"));
}

test("el Knowledge Object de Modelo 151 valida contra el schema publicado", async () => {
...
test("el objeto cumple las reglas semanticas de STORY-KF-008C", async () => {
  const knowledgeObject = await loadKnowledgeObject();
...
test("el contexto IA incluye acceso, permanencia, cronologia y riesgos", async () => {
  const knowledgeObject = await loadKnowledgeObject();
  ...
  assert.equal(
    result.data.technicalDevelopment.some((entry) => /acceso/u.test(entry)),
    true,
  );
  assert.equal(
    result.data.technicalDevelopment.some((entry) => /permanencia/u.test(entry)),
    true,
  );
  assert.equal(
    result.data.technicalDevelopment.some((entry) => /Cronologia operativa/u.test(entry)),
    true,
  );
  assert.equal(
    result.data.technicalDevelopment.some((entry) => /Delegated Analysis/u.test(entry)),
    true,
  );
});
...
test("el objeto integra familiares, Decision Gateway y cronologia operativa", async () => {
  const knowledgeObject = await loadKnowledgeObject();
  ...
  assert.equal(/Decision Gateway/u.test(technicalBlock.content.development), true);
  assert.equal(/Cronologia operativa/u.test(technicalBlock.content.development), true);
  assert.equal(/familiares|conyuge|hijos/u.test(technicalBlock.content.development), true);
  assert.equal(/Delegated Analysis/u.test(technicalBlock.content.development), true);
  assert.equal(
...
test("el informe de validacion queda certificado completamente en PASS", async () => {
  ...
  assert.equal(artifacts.validationReport.story, "STORY-KF-008C");
...
test("los ficheros generados quedan sincronizados con la derivacion actual", async () => {
  ...
  assert.equal(validationReport.story, "STORY-KF-008C");
